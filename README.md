...
The `india-food-ad-scraper` Worker covers 192 Indian food and beverage brands from `scraper-worker/src/brands.js`, with the expanded catalogue focused on digital-first and D2C brands. Its normal refresh runs once a week on Sunday morning in eight 24-brand batches between 06:00 and 09:30 Asia/Kolkata. The batches use one Browser Run session with up to three isolated pages, so the collector finishes in a bounded window instead of running continuously. The earlier MIT-licensed `meta-ads-collector` implementation remains in `scraper/` as a local or self-hosted fallback.
